...
#if/else controls directories for entire workflow based on evaluation & year specified
#Oyster Sanctuary --> density outputs, including breakdown by material type
if (evaluation == 'OS') {
  ...
  df_name <- paste0("OS_", survey_year,"hist_data.csv")
  ...
  OS_obs_clean <- OS_obs_clean %>% select(-Quadrat.Size, -Month, -Day, -Oyster, -Size.Class, -Total.Oyster.Count, -Spat.Y.N, -Algae.Cover, -Sub.Legal.Y.N, -X, -Legal.Y.N,-UID, -LVL)
  ...
  all_dens_output <- paste0(survey_year, "_OS_densities.csv")
  write.csv(all_density, all_dens_output)
  ...
  #output csv for densities grouped by sanctuary & material type
  matOS_dens_output <- paste0("matOS_avgs", survey_year, ".csv")
  write.csv(matOSdens, matOS_dens_output)
  ...
  #output densities by sanctuary
  OSdens_output <- paste0("OS_avgs", survey_year, ".csv")
  write.csv(OSdens, OSdens_output)
  
  #calculate the size class density averages for the entire oyster sanctuary network
  density_OSNavg <- all_density%>%
    summarise(n = n(), 
              avg_total_dens = mean(total_density, na.rm = TRUE),
              sd_total_dens = sd(total_density, na.rm = TRUE),
              avg_legal_dens = mean(legal_density, na.rm = TRUE),
  ...
  #output density for entire OS network observed in survey year
  OSN_output <- paste0("OSN_avg", survey_year, ".csv")
  write.csv(density_OSNavg, OSN_output)
  
} else {
  #DORA density calculations - deepwater oyster recovery areas/natural oyster beds w/ scuba or quadrat surveys
  
  #set folder directories with the year for upload
  data_dir <- paste0("S:/14. DORAs/SCUBA monitoring/", survey_year, "/Analysis")
  df_name <- paste0("DORA_", survey_year, "hist_data.csv")
  
  #upload data
  setwd(data_dir)
  dora_df <- read.csv(df_name)
  
  #clean spaces/symbols from column headers
  names(dora_df) <- make.names(names(dora_df))
  
  dora_df$LVL<-as.numeric(dora_df$LVL)

  # ISOLATE ENVIRONMENTAL DATA
  # Create a dataframe with all the environmental data and Sample ID (SID will be the key for a join)
  environ_data <- dora_df %>%
    select(SID,Collection.Method,Sample.Method, Latitude, Longitude, Oyster.Cover,Mussel.Cover,Sedimentation,Boring.Sponge,Sample.Depth,Max.Depth, Relief, S.Do, B.Do, S.Sal, B.Sal, S.Temp, B.Temp)
  
  # removes duplicate rows, only unique SID remains; dataframe should have the same number of rows as dens_total dataframe
  environ_data <- environ_data[!duplicated(environ_data$SID),]
  
  # Separate sample data by method used - extraction or observation
  extract<-subset(dora_df, dora_df$Collection.Method == "Extraction") #isolate extraction data rows
  extract$LVL <- as.numeric((extract$LVL))
  
  obs <- subset(dora_df, dora_df$Collection.Method == "Observation") #isolate observation data rows
  ...
  # EXTRACTED DENSITIES - work with extracted dataframe to calculate densities
  # Break up the oysters into subclasses; creates DF object for all oyster measurements for legal, sublegal, and spat size classes
  oysters_legal <- filter(extract, extract$LVL >75) #exclusive of 75
  oysters_sublegal <- filter(extract, between(LVL, 26, 75)) #inclusive of 26 and 75
  oysters_spat <- filter(extract, LVL <=25) #inclusive of 25
  
  # Function to calculate oyster density 
  oyster_density <- function(x) {
    (x)%>%
      group_by(Year, DORA, Region, Site_ID,SID)%>%
      dplyr::summarise(oyst_density =4*n ())
  }  
  ...
  #total density across all size classes, join environmental data to each unique sample
  dens_total <- oyster_density(extract)
  dens_total <- left_join(dens_total,environ_data,by="SID")
  #write.csv(dens_total,"S:/8. Oyster Sanctuaries/3. Monitoring and Data/1. Oyster Sanctuary (OS)/5. Analysis/2025 R/excavated densities/dens_total.csv", row.names=FALSE)
  ...
  #remove unecessary rows for a density dataframe
  obs_clean <- obs_clean %>% select(-Quadrat.Size, -Month, -Day, -Oyster, -Size.Class, -Total.Oyster.Count, -UID, -LVL)
  
  #merge density dataframes together
  all_density<- rbind(obs_clean, extract_density)
  
  #clean up variables in the workspace/global environment
  rm(dens_legal,dens_spat,dens_sublegal,dens_total,oysters_legal,oysters_spat,oysters_sublegal)
  
  #create a csv file with site specific density estimates (each quadrat/dive has an associated series of size class densities)
  output_name <- paste0("DORA_densities", survey_year, ".csv")
  write.csv(all_density, output_name)
